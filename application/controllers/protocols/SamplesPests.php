...
class SamplesPests extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_PESTS;
  ...
  private function prepareFields(){
    ...
    $this->data['input_pest_id']        = fl_pest_id($this->session->flashdata('set_value_pest_id'), $this->session->flashdata('set_error_pest_id'));

    //listas de apoio
    $laParams = array('license_id' => fi_get_license());
    $this->data['input_pests_list'] = $this->ds_model->get_list_pests($laParams);
  }
  ...
  private function getRecord($poRecord){
    ...
    $this->data['pest_id']      = $poRecord[0]->pest_id;
  }
  ...
  private function postRecord(){
    ...
    $this->data['pest_id']      = trim(xss_clean($this->input->post('edt_pest_id')));
  }
  ...
  private function flashRecord(){
    ...
    $this->session->set_flashdata('set_value_pest_id',      $this->data['pest_id']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_pest_id',      form_error('edt_pest_id'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_pest_id()));
  ...
  private function saveRecord(){
    ...
    $this->record['pest_id']        = $this->data['pest_id'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesPests_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Pragas';
    $this->data['view_title_rec'] = 'Amostra de Praga';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_PESTS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_PESTS_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_PESTS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_pests/vw_samples_pests');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_pests/vw_samples_pests_ins', $this->data);
    $this->load->view('protocols/samples_pests/js_samples_pests');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_PESTS_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_pests/vw_samples_pests_upd', $this->data);
    $this->load->view('protocols/samples_pests/js_samples_pests');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_PESTS_VW_UPD).'/'.md5($this->data['sample_id']));
      }else{
        self::saveRecord();
