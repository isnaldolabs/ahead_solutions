...
class ProtocolsGuPlots extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_PROTOCOLS_GU_PLOTS;

  private function prepareFields(){
    $this->data['input_id']             = fl_id($this->session->flashdata('set_value_id'), $this->session->flashdata('set_error_id'));
    $this->data['input_plot_id']        = fl_protocol_plot_id($this->session->flashdata('set_value_plot_id'), $this->session->flashdata('set_error_plot_id'));
    ...
    $this->data['input_plot_list'] = $this->ds_model->get_list_gu_plots($laParams);
    log_message('info', $this->db->last_query());
  }

  private function getRecord($poRecord){
    $this->data['id']           = $poRecord[0]->id;
  ...
  private function postRecord(){
    ...
    $this->data['plot_id']      = trim(xss_clean($this->input->post('edt_protocol_plot_id')));
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/ProtocolsGuPlots_model', 'ds_model');

    $this->data['view_title'] = 'Talhões do Protocolo';
    $this->data['view_title_rec'] = 'Talhão do Protocolo';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_PROTOCOLS_GU_PLOTS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_PROTOCOLS_GU_PLOTS_VW_INS);
    $this->data['link_update'] = base_url(RT_PROTOCOLS_GU_PLOTS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/protocols_gu_plots/vw_protocols_gu_plots');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/protocols_gu_plots/vw_protocols_gu_plots_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PROTOCOLS_GU_PLOTS_VW_INS));
      }else{
        self::saveRecord();
