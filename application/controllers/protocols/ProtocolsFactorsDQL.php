...
class ProtocolsFactorsDQL extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_PROTOCOLS_FACTORS_DQL;

  private function prepareFields(){
    $this->data['input_factor_id']      = fl_factor_id($this->session->flashdata('set_value_factor_id'), $this->session->flashdata('set_error_factor_id'));
    ...
    $this->data['input_factor_id_c']    = fl_factor_id_c($this->session->flashdata('set_value_factor_id_c'), $this->session->flashdata('set_error_factor_id_c'));
    $this->data['input_factor_id_q']    = fl_factor_id_q($this->session->flashdata('set_value_factor_id_q'), $this->session->flashdata('set_error_factor_id_q'));
  ...
  private function postRecord(){
    $this->data['factor_id']    = trim(xss_clean($this->input->post('edt_factor_id')));
    ...
    $this->data['factor_id_c']  = trim(xss_clean($this->input->post('edt_factor_id_c')));
    $this->data['factor_id_q']  = trim(xss_clean($this->input->post('edt_factor_id_q')));
  }

  private function flashRecord(){
    $this->session->set_flashdata('set_value_factor_id',    $this->data['factor_id']);
    ...
    $this->session->set_flashdata('set_value_factor_id_c',  $this->data['factor_id_c']);
    $this->session->set_flashdata('set_value_factor_id_q',  $this->data['factor_id_q']);
  }

  private function flashErrors(){
    $this->session->set_flashdata('set_error_factor_id',    form_error('edt_factor_id'));
    ...
    $this->session->set_flashdata('set_error_factor_id_c',  form_error('edt_factor_id_c'));
    $this->session->set_flashdata('set_error_factor_id_q',  form_error('edt_factor_id_q'));
  }
  ...
  private function saveRecord(){
    if(isset($this->data['factor_id'])){
      $this->record['factor_id'] = $this->data['factor_id'];
    }
    $this->record['protocol_id']    = fi_get_protocol();
    $this->record['license_id']     = fi_get_license();
    $this->record['factor_id_c']    = $this->data['factor_id_c'];
    $this->record['factor_id_q']    = $this->data['factor_id_q'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/ProtocolsFactorsDQL_model', 'ds_model');

    $this->data['view_title'] = 'Fatores D.Q.L. do Protocolo';
    $this->data['view_title_rec'] = 'Fator D.Q.L. do Protocolo';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_PROTOCOLS_FACTORS_DQL_DB_DEL);
    $this->data['link_insert'] = base_url(RT_PROTOCOLS_FACTORS_DQL_VW_INS);
    $this->data['link_update'] = base_url(RT_PROTOCOLS_FACTORS_DQL_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/protocols_factors_dql/vw_protocols_factors_dql');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/protocols_factors_dql/vw_protocols_factors_dql_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PROTOCOLS_FACTORS_DQL_VW_INS));
      }else{
        self::saveRecord();
