...
class SamplesAnalyze extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_ANALYZE;

  private function prepareFields(){
    $this->data['input_analyze_id']     = fl_analyze_id($this->session->flashdata('set_value_analyze_id'), $this->session->flashdata('set_error_analyze_id'));
    ...
    $this->data['input_num_weight']     = fl_num_weight($this->session->flashdata('set_value_num_weight'), $this->session->flashdata('set_error_num_weight'));
    $this->data['input_num_brix']       = fl_num_brix($this->session->flashdata('set_value_num_brix'), $this->session->flashdata('set_error_num_brix'));
    $this->data['input_num_lai']        = fl_num_lai($this->session->flashdata('set_value_num_lai'), $this->session->flashdata('set_error_num_lai'));
    $this->data['input_num_pbu']        = fl_num_pbu($this->session->flashdata('set_value_num_pbu'), $this->session->flashdata('set_error_num_pbu'));
  }

  private function getRecord($poRecord){
    $lr_num_weight  = str_replace(".",",", $poRecord[0]->num_weight);
    $lr_num_brix    = str_replace(".",",", $poRecord[0]->num_brix);
    $lr_num_lai     = str_replace(".",",", $poRecord[0]->num_lai);
    $lr_num_pbu     = str_replace(".",",", $poRecord[0]->num_pbu);

    $this->data['analyze_id']   = $poRecord[0]->analyze_id;
    ...
    $this->data['num_weight']   = $lr_num_weight;
    $this->data['num_brix']     = $lr_num_brix;
    $this->data['num_lai']      = $lr_num_lai;
    $this->data['num_pbu']      = $lr_num_pbu;
  }

  private function postRecord(){
    $lr_num_weight  = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_weight')))));
    $lr_num_brix    = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_brix')))));
    $lr_num_lai     = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_lai')))));
    $lr_num_pbu     = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_pbu')))));

    $this->data['analyze_id']   = trim(xss_clean($this->input->post('edt_analyze_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_analyze_id',   $this->data['analyze_id']);
    ...
    $this->session->set_flashdata('set_value_num_weight',   $this->data['num_weight']);
    $this->session->set_flashdata('set_value_num_brix',     $this->data['num_brix']);
    $this->session->set_flashdata('set_value_num_lai',      $this->data['num_lai']);
    $this->session->set_flashdata('set_value_num_pbu',      $this->data['num_pbu']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_num_weight',   form_error('edt_num_weight'));
    $this->session->set_flashdata('set_error_num_brix',     form_error('edt_num_brix'));
    $this->session->set_flashdata('set_error_num_lai',      form_error('edt_num_lai'));
    $this->session->set_flashdata('set_error_num_pbu',      form_error('edt_num_pbu'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_brix(), rule_num_lai(), rule_num_pbu()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['analyze_id'])){
      $this->record['analyze_id'] = $this->data['analyze_id'];
    }
    ...
    $this->record['num_weight']     = $this->data['num_weight'];
    $this->record['num_brix']       = $this->data['num_brix'];
    $this->record['num_lai']        = $this->data['num_lai'];
    $this->record['num_pbu']        = $this->data['num_pbu'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesAnalyze_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Pré-Análise';
    $this->data['view_title_rec'] = 'Amostra de Pré-Análise';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_ANALYZE_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_ANALYZE_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_ANALYZE_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_analyze/vw_samples_analyze');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_analyze/vw_samples_analyze_ins', $this->data);
    $this->load->view('protocols/samples_analyze/js_samples_analyze');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_ANALYZE_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_analyze_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->analyze_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_analyze/vw_samples_analyze_upd', $this->data);
    $this->load->view('protocols/samples_analyze/js_samples_analyze');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_ANALYZE_VW_UPD).'/'.md5($this->data['analyze_id']));
      }else{
        self::saveRecord();
