...
class SamplesFlowering extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_FLOWERING;

  private function prepareFields(){
    $this->data['input_flower_id']      = fl_flower_id($this->session->flashdata('set_value_flower_id'), $this->session->flashdata('set_error_flower_id'));
    ...
    $this->data['input_num_flower']     = fl_num_flower($this->session->flashdata('set_value_num_flower'), $this->session->flashdata('set_error_num_flower'));
  }

  private function getRecord($poRecord){
    $lr_num_flower  = str_replace(".",",", $poRecord[0]->num_flower);

    $this->data['flower_id']    = $poRecord[0]->flower_id;
    ...
    $this->data['num_flower']   = $lr_num_flower;
  }

  private function postRecord(){
    $lr_num_flower  = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_flower')))));

    $this->data['flower_id']    = trim(xss_clean($this->input->post('edt_flower_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_flower_id',    $this->data['flower_id']);
    ...
    $this->session->set_flashdata('set_value_num_flower',   $this->data['num_flower']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_num_flower',   form_error('edt_num_flower'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_flower()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['flower_id'])){
      $this->record['flower_id'] = $this->data['flower_id'];
    }
    ...
    $this->record['num_flower']     = $this->data['num_flower'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesFlowering_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Florescimento';
    $this->data['view_title_rec'] = 'Amostra de Florescimento';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_FLOWERING_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_FLOWERING_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_FLOWERING_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_flowering/vw_samples_flowering');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_flowering/vw_samples_flowering_ins', $this->data);
    $this->load->view('protocols/samples_flowering/js_samples_flowering');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_FLOWERING_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_flower_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->flower_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_flowering/vw_samples_flowering_upd', $this->data);
    $this->load->view('protocols/samples_flowering/js_samples_flowering');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_FLOWERING_VW_UPD).'/'.md5($this->data['flower_id']));
      }else{
        self::saveRecord();
