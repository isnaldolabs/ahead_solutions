...
class SamplesHeights extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_HEIGHTS;

  private function prepareFields(){
    $this->data['input_height_id']      = fl_height_id($this->session->flashdata('set_value_height_id'), $this->session->flashdata('set_error_height_id'));
    ...
    $this->data['input_num_height']     = fl_num_height($this->session->flashdata('set_value_num_height'), $this->session->flashdata('set_error_num_height'));
  }

  private function getRecord($poRecord){
    $lr_num_height  = str_replace(".",",", $poRecord[0]->num_height);

    $this->data['height_id']    = $poRecord[0]->height_id;
    ...
    $this->data['num_height']   = $lr_num_height;
  }

  private function postRecord(){
    $lr_num_height  = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_height')))));

    $this->data['height_id']    = trim(xss_clean($this->input->post('edt_height_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_height_id',    $this->data['height_id']);
    ...
    $this->session->set_flashdata('set_value_num_height',   $this->data['num_height']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_num_height',   form_error('edt_num_height'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_height()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['height_id'])){
      $this->record['height_id'] = $this->data['height_id'];
    }
    ...
    $this->record['num_height']     = $this->data['num_height'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesHeights_model', 'ds_model');

    $this->data['view_title'] = 'Amostras das Alturas';
    $this->data['view_title_rec'] = 'Altura';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_HEIGHTS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_HEIGHTS_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_HEIGHTS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_heights/vw_samples_heights');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_heights/vw_samples_heights_ins', $this->data);
    $this->load->view('protocols/samples_heights/js_samples_heights');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_HEIGHTS_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_height_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->height_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_heights/vw_samples_heights_upd', $this->data);
    $this->load->view('protocols/samples_heights/js_samples_heights');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_HEIGHTS_VW_UPD).'/'.md5($this->data['height_id']));
      }else{
        self::saveRecord();
