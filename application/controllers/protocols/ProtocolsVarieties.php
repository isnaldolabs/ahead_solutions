...
class ProtocolsVarieties extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_PROTOCOLS_VARIETIES;
  ...
  private function prepareFields(){
    ...
    $this->data['input_variety_list'] = $this->ds_model->get_list_parcels_varieties($laParams);
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/ProtocolsVarieties_model', 'ds_model');

    $this->data['view_title'] = 'Tratamento Varietal';
    $this->data['view_title_rec'] = 'Variedade para Tratamento';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_PROTOCOLS_VARIETIES_DB_DEL);
    $this->data['link_insert'] = base_url(RT_PROTOCOLS_VARIETIES_VW_INS);
    $this->data['link_update'] = base_url(RT_PROTOCOLS_VARIETIES_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/protocols_varieties/vw_protocols_varieties');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/protocols_varieties/vw_protocols_varieties_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PROTOCOLS_VARIETIES_VW_INS));
      }else{
        self::saveRecord();
