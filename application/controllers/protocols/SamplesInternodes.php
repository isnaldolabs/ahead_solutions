...
class SamplesInternodes extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_INTERNODES;

  private function prepareFields(){
    $this->data['input_node_id']        = fl_node_id($this->session->flashdata('set_value_node_id'), $this->session->flashdata('set_error_node_id'));
    ...
    $this->data['input_num_node']       = fl_num_node($this->session->flashdata('set_value_num_node'), $this->session->flashdata('set_error_num_node'));
  }

  private function getRecord($poRecord){
    $lr_num_node  = str_replace(".",",", $poRecord[0]->num_node);

    $this->data['node_id']      = $poRecord[0]->node_id;
    ...
    $this->data['num_node']     = $lr_num_node;
  }

  private function postRecord(){
    $lr_num_node  = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_node')))));

    $this->data['node_id']      = trim(xss_clean($this->input->post('edt_node_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_node_id',      $this->data['node_id']);
    ...
    $this->session->set_flashdata('set_value_num_node',     $this->data['num_node']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_num_node',     form_error('edt_num_node'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_node()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['node_id'])){
      $this->record['node_id'] = $this->data['node_id'];
    }
    ...
    $this->record['num_node']       = $this->data['num_node'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesInternodes_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Entrenós';
    $this->data['view_title_rec'] = 'Amostra de Entrenós';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_INTERNODES_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_INTERNODES_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_INTERNODES_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_internodes/vw_samples_internodes');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_internodes/vw_samples_internodes_ins', $this->data);
    $this->load->view('protocols/samples_internodes/js_samples_internodes');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_INTERNODES_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_node_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->node_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_internodes/vw_samples_internodes_upd', $this->data);
    $this->load->view('protocols/samples_internodes/js_samples_internodes');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_INTERNODES_VW_UPD).'/'.md5($this->data['node_id']));
      }else{
        self::saveRecord();
