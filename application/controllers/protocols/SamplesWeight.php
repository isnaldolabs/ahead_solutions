...
class SamplesWeight extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_WEIGHT;

  private function prepareFields(){
    $this->data['input_weight_id']      = fl_weight_id($this->session->flashdata('set_value_weight_id'), $this->session->flashdata('set_error_weight_id'));
  ...
  private function getRecord($poRecord){
    ...
    $this->data['weight_id']    = $poRecord[0]->weight_id;
  ...
  private function postRecord(){
    ...
    $this->data['weight_id']    = trim(xss_clean($this->input->post('edt_weight_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_weight_id',    $this->data['weight_id']);
  ...
  private function checkRecord(){
    ...
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_weight()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['weight_id'])){
      $this->record['weight_id'] = $this->data['weight_id'];
    }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesWeight_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Peso';
    $this->data['view_title_rec'] = 'Amostra de Peso';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_WEIGHT_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_WEIGHT_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_WEIGHT_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_weight/vw_samples_weight');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_weight/vw_samples_weight_ins', $this->data);
    $this->load->view('protocols/samples_weight/js_samples_weight');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_WEIGHT_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_weight_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->weight_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_weight/vw_samples_weight_upd', $this->data);
    $this->load->view('protocols/samples_weight/js_samples_weight');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_WEIGHT_VW_UPD).'/'.md5($this->data['weight_id']));
      }else{
        self::saveRecord();
