...
class SamplesPith extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_PITH;

  private function prepareFields(){
    $this->data['input_pith_id']        = fl_pith_id($this->session->flashdata('set_value_pith_id'), $this->session->flashdata('set_error_pith_id'));
    ...
    $this->data['input_num_pith']       = fl_num_pith($this->session->flashdata('set_value_num_pith'), $this->session->flashdata('set_error_num_pith'));
  }

  private function getRecord($poRecord){
    $lr_num_pith  = str_replace(".",",", $poRecord[0]->num_pith);

    $this->data['pith_id']      = $poRecord[0]->pith_id;
    ...
    $this->data['num_pith']     = $lr_num_pith;
  }

  private function postRecord(){
    $lr_num_pith  = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_pith')))));

    $this->data['pith_id']      = trim(xss_clean($this->input->post('edt_pith_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_pith_id',      $this->data['pith_id']);
    ...
    $this->session->set_flashdata('set_value_num_pith',     $this->data['num_pith']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_num_pith',     form_error('edt_num_pith'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_pith()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['pith_id'])){
      $this->record['pith_id'] = $this->data['pith_id'];
    }
    ...
    $this->record['num_pith']       = $this->data['num_pith'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesPith_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Isoporização';
    $this->data['view_title_rec'] = 'Amostra de Isoporização';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_PITH_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_PITH_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_PITH_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_pith/vw_samples_pith');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_pith/vw_samples_pith_ins', $this->data);
    $this->load->view('protocols/samples_pith/js_samples_pith');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_PITH_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_pith_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->pith_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_pith/vw_samples_pith_upd', $this->data);
    $this->load->view('protocols/samples_pith/js_samples_pith');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_PITH_VW_UPD).'/'.md5($this->data['pith_id']));
      }else{
        self::saveRecord();
