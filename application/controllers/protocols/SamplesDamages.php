...
class SamplesDamages extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_DAMAGES;

  private function prepareFields(){
    $this->data['input_damage_id']      = fl_damage_id($this->session->flashdata('set_value_damage_id'), $this->session->flashdata('set_error_damage_id'));
    ...
    $this->data['input_num_damage']     = fl_num_damage($this->session->flashdata('set_value_num_damage'), $this->session->flashdata('set_error_num_damage'));
  }

  private function getRecord($poRecord){
    $lr_num_damage = str_replace(".",",", $poRecord[0]->num_damage);

    $this->data['damage_id']    = $poRecord[0]->damage_id;
    ...
    $this->data['num_damage']   = $lr_num_damage;
  }

  private function postRecord(){
    $lr_num_damage = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_damage')))));

    $this->data['damage_id']    = trim(xss_clean($this->input->post('edt_damage_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_damage_id',    $this->data['damage_id']);
    ...
    $this->session->set_flashdata('set_value_num_damage',   $this->data['num_damage']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_num_damage',   form_error('edt_num_damage'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_damage()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['damage_id'])){
      $this->record['damage_id'] = $this->data['damage_id'];
    }
    ...
    $this->record['num_damage']     = $this->data['num_damage'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesDamages_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Danos';
    $this->data['view_title_rec'] = 'Amostra de Danos';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_DAMAGES_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_DAMAGES_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_DAMAGES_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_damages/vw_samples_damages');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_damages/vw_samples_damages_ins', $this->data);
    $this->load->view('protocols/samples_damages/js_samples_damages');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_DAMAGES_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_damage_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->damage_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_damages/vw_samples_damages_upd', $this->data);
    $this->load->view('protocols/samples_damages/js_samples_damages');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_DAMAGES_VW_UPD).'/'.md5($this->data['damage_id']));
      }else{
        self::saveRecord();
