...
class SamplesHoles extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_HOLES;

  private function prepareFields(){
    $this->data['input_hole_id']        = fl_hole_id($this->session->flashdata('set_value_hole_id'), $this->session->flashdata('set_error_hole_id'));
    ...
    $this->data['input_num_hole']       = fl_num_hole($this->session->flashdata('set_value_num_hole'), $this->session->flashdata('set_error_num_hole'));
  }

  private function getRecord($poRecord){
    $lr_num_hole = str_replace(".",",", $poRecord[0]->num_hole);

    $this->data['hole_id']      = $poRecord[0]->hole_id;
    ...
    $this->data['num_hole']     = $lr_num_hole;
  }

  private function postRecord(){
    $lr_num_hole = str_replace(",",".", str_replace(".","", trim(xss_clean($this->input->post('edt_num_hole')))));

    $this->data['hole_id']      = trim(xss_clean($this->input->post('edt_hole_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_hole_id',      $this->data['hole_id']);
    ...
    $this->session->set_flashdata('set_value_num_hole',     $this->data['num_hole']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_num_hole',     form_error('edt_num_hole'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_hole()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['hole_id'])){
      $this->record['hole_id'] = $this->data['hole_id'];
    }
    ...
    $this->record['num_hole']       = $this->data['num_hole'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesHoles_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Perfurados';
    $this->data['view_title_rec'] = 'Amostra de Perfurados';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_HOLES_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_HOLES_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_HOLES_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_holes/vw_samples_holes');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_holes/vw_samples_holes_ins', $this->data);
    $this->load->view('protocols/samples_holes/js_samples_holes');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_HOLES_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_hole_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->hole_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_holes/vw_samples_holes_upd', $this->data);
    $this->load->view('protocols/samples_holes/js_samples_holes');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_HOLES_VW_UPD).'/'.md5($this->data['hole_id']));
      }else{
        self::saveRecord();
