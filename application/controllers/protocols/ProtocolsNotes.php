...
class ProtocolsNotes extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_PROTOCOLS_NOTES;

  private function prepareFields(){
    $this->data['input_note_id']        = fl_note_id($this->session->flashdata('set_value_note_id'), $this->session->flashdata('set_error_note_id'));
    ...
    $this->data['input_note_at']        = fl_note_at($this->session->flashdata('set_value_note_at'), $this->session->flashdata('set_error_note_at'));
  ...
  private function postRecord(){
    ...
    $this->data['note_at']      = $this->input->post('edt_note_at');
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_note_id',      $this->data['note_id']);
    ...
    $this->session->set_flashdata('set_value_note_at',      $this->data['note_at']);
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_note_at',      form_error('edt_note_at'));
  ...
  private function checkRecord(){
    ...
    $this->form_validation->set_rules(array(rule_notes(), rule_note_at()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['note_id'])){
      $this->record['note_id'] = $this->data['note_id'];
    }
    $this->record['protocol_id']    = fi_get_protocol();
    $this->record['license_id']     = fi_get_license();
    $this->record['user_id']        = fi_get_user();
    $this->record['note_at']        = $this->data['note_at'];
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/ProtocolsNotes_model', 'ds_model');

    $this->data['view_title']   = 'Observações';
    $this->data['view_title_rec'] = 'Observação';
    $this->data['menu_active']  = MENU_PROTOCOLS;
    $this->data['link_delete']  = base_url(RT_PROTOCOLS_NOTES_DB_DEL);
    $this->data['link_insert']  = base_url(RT_PROTOCOLS_NOTES_VW_INS);
    $this->data['link_update']  = base_url(RT_PROTOCOLS_NOTES_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/protocols_notes/vw_protocols_notes');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/protocols_notes/vw_protocols_notes_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PROTOCOLS_NOTES_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_note_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->note_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/protocols_notes/vw_protocols_notes_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PROTOCOLS_NOTES_VW_UPD).'/'.md5($this->data['note_id']));
      }else{
        self::saveRecord();
        if($this->ds_model->upd($this->record)){
          ...
          $this->session->set_flashdata(MSG_EVENT, array('type'=>TYPE_DANGER, 'text'=>MSG_FAILURE_SAVING));
          log_message('error', MSG_FAILURE_SAVING);
          redirect(base_url(RT_PROTOCOLS_NOTES_VW_UPD).'/'.md5($this->data['id']));
        }
      }
    }
  }
