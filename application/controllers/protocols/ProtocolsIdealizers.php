...
class ProtocolsIdealizers extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_PROTOCOLS_IDEALIZERS;

  private function prepareFields(){
    $this->data['input_id']             = fl_id($this->session->flashdata('set_value_id'), $this->session->flashdata('set_error_id'));
    $this->data['input_idealizer_id']   = fl_idealizer_id($this->session->flashdata('set_value_idealizer_id'), $this->session->flashdata('set_error_idealizer_id'));
    ...
    $this->data['input_idealizer_list'] = $this->ds_model->get_list_idealizers($laParams);
  }

  private function postRecord(){
    $this->data['id']           = trim(xss_clean($this->input->post('edt_id')));
    $this->data['idealizer_id'] = trim(xss_clean($this->input->post('edt_idealizer_id')));
  ...
  private function flashRecord(){
    ...
    $this->session->set_flashdata('set_value_idealizer_id', $this->data['idealizer_id']);
  ...
  private function saveRecord(){
    ...
    $this->record['idealizer_id']   = $this->data['idealizer_id'];
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/ProtocolsIdealizers_model', 'ds_model');

    $this->data['view_title'] = 'Idealizadores do Protocolo';
    $this->data['view_title_rec'] = 'Idealizador do Protocolo';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_PROTOCOLS_IDEALIZERS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_PROTOCOLS_IDEALIZERS_VW_INS);
    $this->data['link_update'] = base_url(RT_PROTOCOLS_IDEALIZERS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/protocols_idealizers/vw_protocols_idealizers');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/protocols_idealizers/vw_protocols_idealizers_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PROTOCOLS_IDEALIZERS_VW_INS));
      }else{
        self::saveRecord();
