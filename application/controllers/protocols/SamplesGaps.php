...
class SamplesGaps extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_GAPS;

  private function prepareFields(){
    $this->data['input_gap_id']         = fl_gap_id($this->session->flashdata('set_value_gap_id'), $this->session->flashdata('set_error_gap_id'));
  ...
  private function getRecord($poRecord){
    ...
    $this->data['gap_id']       = $poRecord[0]->gap_id;
  ...
  private function postRecord(){
    ...
    $this->data['gap_id']       = trim(xss_clean($this->input->post('edt_gap_id')));
  ...
  private function flashRecord(){
    $this->session->set_flashdata('set_value_gap_id',       $this->data['gap_id']);
  ...
  private function checkRecord(){
    ...
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_gap()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['gap_id'])){
      $this->record['gap_id'] = $this->data['gap_id'];
    }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesGaps_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Falhas';
    $this->data['view_title_rec'] = 'Amostra de Falhas';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_GAPS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_GAPS_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_GAPS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_gaps/vw_samples_gaps');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_gaps/vw_samples_gaps_ins', $this->data);
    $this->load->view('protocols/samples_gaps/js_samples_gaps');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_GAPS_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_gap_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->gap_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_gaps/vw_samples_gaps_upd', $this->data);
    $this->load->view('protocols/samples_gaps/js_samples_gaps');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_GAPS_VW_UPD).'/'.md5($this->data['gap_id']));
      }else{
        self::saveRecord();
