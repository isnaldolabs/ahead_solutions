...
class ProtocolsSamples extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_PROTOCOLS_SAMPLES;
  ...
  private function prepareFields(){
    ...
    $this->data['input_sample_list'] = $this->ds_model->get_list_samples($laParams);
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/ProtocolsSamples_model', 'ds_model');

    $this->data['view_title'] = 'Amostras do Protocolo';
    $this->data['view_title_rec'] = 'Amostras do Protocolo';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_PROTOCOLS_SAMPLES_DB_DEL);
    $this->data['link_insert'] = base_url(RT_PROTOCOLS_SAMPLES_VW_INS);
  ...
  public function index(){
    ...
    $this->load->view('protocols/protocols_samples/vw_protocols_samples');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/protocols_samples/vw_protocols_samples_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PROTOCOLS_SAMPLES_VW_INS));
      }else{
        self::saveRecord();
