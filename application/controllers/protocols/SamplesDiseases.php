...
class SamplesDiseases extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_DISEASES;
  ...
  private function prepareFields(){
    ...
    $this->data['input_diseases_list'] = $this->ds_model->get_list_diseases($laParams);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_disease_id',   form_error('edt_disease_id'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_disease_id()));
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesDiseases_model', 'ds_model');

    $this->data['view_title'] = 'Amostras de Doenças';
    $this->data['view_title_rec'] = 'Amostra de Doença';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_DISEASES_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_DISEASES_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_DISEASES_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_diseases/vw_samples_diseases');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_diseases/vw_samples_diseases_ins', $this->data);
    $this->load->view('protocols/samples_diseases/js_samples_diseases');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_DISEASES_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_diseases/vw_samples_diseases_upd', $this->data);
    $this->load->view('protocols/samples_diseases/js_samples_diseases');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_DISEASES_VW_UPD).'/'.md5($this->data['sample_id']));
      }else{
        self::saveRecord();
