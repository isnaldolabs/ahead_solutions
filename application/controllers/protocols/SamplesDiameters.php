...
class SamplesDiameters extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_SAMPLES_DIAMETERS;

  private function prepareFields(){
    $this->data['input_diameter_id']    = fl_diameter_id($this->session->flashdata('set_value_diameter_id'), $this->session->flashdata('set_error_diameter_id'));
    ...
    $this->data['input_num_diameter']   = fl_num_diameter($this->session->flashdata('set_value_num_diameter'), $this->session->flashdata('set_error_num_diameter'));
  }

  private function getRecord($poRecord){
    $this->data['diameter_id']  = $poRecord[0]->diameter_id;
    ...
    $this->data['num_diameter'] = $poRecord[0]->num_diameter;
  }

  private function postRecord(){
    $this->data['diameter_id']  = trim(xss_clean($this->input->post('edt_diameter_id')));
    ...
    $this->data['num_diameter'] = trim(xss_clean($this->input->post('edt_num_diameter')));
  }

  private function flashRecord(){
    $this->session->set_flashdata('set_value_diameter_id',  $this->data['diameter_id']);
    ...
    $this->session->set_flashdata('set_value_num_diameter', $this->data['num_diameter']);
  }
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_num_diameter', form_error('edt_num_diameter'));
  }

  private function checkRecord(){
    $this->form_validation->set_error_delimiters(HTML_ERROR_PREFIX, HTML_ERROR_SUFFIX);
    $this->form_validation->set_rules(array(rule_dt_sample(), rule_spot_id(), rule_num_diameter()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['diameter_id'])){
      $this->record['diameter_id'] = $this->data['diameter_id'];
    }
    ...
    $this->record['num_diameter']   = $this->data['num_diameter'];
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('protocols/SamplesDiameters_model', 'ds_model');

    $this->data['view_title'] = 'Amostras dos Diâmetros';
    $this->data['view_title_rec'] = 'Diâmetro';
    $this->data['menu_active'] = MENU_PROTOCOLS;
    $this->data['link_delete'] = base_url(RT_SAMPLES_DIAMETERS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_SAMPLES_DIAMETERS_VW_INS);
    $this->data['link_update'] = base_url(RT_SAMPLES_DIAMETERS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('protocols/samples_diameters/vw_samples_diameters');
  ...
  public function ct_vw_insert($pi_sketch){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_diameters/vw_samples_diameters_ins', $this->data);
    $this->load->view('protocols/samples_diameters/js_samples_diameters');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_DIAMETERS_VW_INS).'/'.$this->data['sketch_id']);
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_diameter_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->diameter_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('protocols/samples_diameters/vw_samples_diameters_upd', $this->data);
    $this->load->view('protocols/samples_diameters/js_samples_diameters');
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_SAMPLES_DIAMETERS_VW_UPD).'/'.md5($this->data['diameter_id']));
      }else{
        self::saveRecord();
