...
class Products extends CI_Controller {
  ...
  protected $link_redirect = RT_PRODUCTS;
  ...
  private function prepareFields(){
    ...
    $this->data['input_manufacturer_id']= fl_manufacturer_id($this->session->flashdata('set_value_manufacturer_id'), $this->session->flashdata('set_error_manufacturer_id'));
    ...
    $this->data['input_manufacturer_list']      = $this->ds_model->get_list_manufacturers($laParams);
    $this->data['input_products_groups_list']   = $this->ds_model->get_list_products_groups($laParams);
  ...
  private function getRecord($poRecord){
    ...
    $this->data['manufacturer_id']  = $poRecord[0]->manufacturer_id;
  ...
  private function postRecord(){
    ...
    $this->data['manufacturer_id']  = trim(xss_clean($this->input->post('edt_manufacturer_id')));
  ...
  private function flashRecord(){
    ...
    $this->session->set_flashdata('set_value_manufacturer_id',  $this->data['manufacturer_id']);
  ...
  private function flashErrors(){
    ...
    $this->session->set_flashdata('set_error_manufacturer_id',  form_error('edt_manufacturer_id'));
  ...
  private function saveRecord(){
    ...
    $this->record['manufacturer_id']    = $this->data['manufacturer_id'];
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/Product_model', 'ds_model');

    $this->data['view_title'] = 'Produtos';
    $this->data['view_title_rec'] = 'Produto';
    $this->data['menu_active'] = MENU_PREPARE;
    $this->data['link_delete'] = base_url(RT_PRODUCTS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_PRODUCTS_VW_INS);
    $this->data['link_update'] = base_url(RT_PRODUCTS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/products/vw_products');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/products/vw_products_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PRODUCTS_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_product_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->product_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/products/vw_products_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PRODUCTS_VW_UPD).'/'.md5($this->data['product_id']));
      }else{
        self::saveRecord();
