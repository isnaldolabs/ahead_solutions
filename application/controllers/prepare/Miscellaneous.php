...
class Miscellaneous extends CI_Controller {
  ...
  protected $link_redirect = RT_MISCELLANEOUS;
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/Miscellaneous_model', 'ds_model');

    $this->data['view_title'] = 'Experimentos Diversos';
    $this->data['view_title_rec'] = 'Experimento Diverso';
    $this->data['menu_active'] = MENU_PREPARE;
    $this->data['link_delete'] = base_url(RT_MISCELLANEOUS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_MISCELLANEOUS_VW_INS);
    $this->data['link_update'] = base_url(RT_MISCELLANEOUS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/miscellaneous/vw_miscellaneous');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/miscellaneous/vw_miscellaneous_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_MISCELLANEOUS_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_misc_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->misc_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/miscellaneous/vw_miscellaneous_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_MISCELLANEOUS_VW_UPD).'/'.md5($this->data['misc_id']));
      }else{
        self::saveRecord();
