...
class ApplicationModes extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_APPLICATION_MODES;
  ...
  private function getRecord($poRecord){
    $this->data['mode_id']    = $poRecord[0]->mode_id;
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/ApplicationMode_model', 'ds_model');

    $this->data['view_title'] = 'Modos de Aplicação';
    $this->data['view_title_rec'] = 'Modo de Aplicação';
    $this->data['menu_active'] = MENU_PREPARE;
    $this->data['link_delete'] = base_url(RT_APPLICATION_MODES_DB_DEL);
    $this->data['link_insert'] = base_url(RT_APPLICATION_MODES_VW_INS);
    $this->data['link_update'] = base_url(RT_APPLICATION_MODES_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/application_modes/vw_application_modes');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/application_modes/vw_application_modes_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_APPLICATION_MODES_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_mode_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->mode_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/application_modes/vw_application_modes_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_APPLICATION_MODES_VW_UPD).'/'.md5($this->data['mode_id']));
      }else{
        self::saveRecord();
