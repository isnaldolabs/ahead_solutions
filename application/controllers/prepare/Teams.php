...
class Teams extends CI_Controller {
  ...
  protected $link_redirect = RT_TEAMS;
  ...
  private function getRecord($poRecord){
    $this->data['team_id']      = $poRecord[0]->team_id;
  ...
  private function checkRecord(){
    ...
    //$this->form_validation->set_rules('edt_name', 'News title', 'validar_teams');

    $this->form_validation->set_rules(array(rule_name50(), rule_email()));
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/Team_model', 'ds_model');

    $this->data['view_title'] = 'Equipe Técnica';
    $this->data['view_title_rec'] = 'Equipe Técnica';
    $this->data['menu_active'] = MENU_PREPARE;
    $this->data['link_delete'] = base_url(RT_TEAMS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_TEAMS_VW_INS);
    $this->data['link_update'] = base_url(RT_TEAMS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/teams/vw_teams');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/teams/vw_teams_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_TEAMS_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_team_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->team_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/teams/vw_teams_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_TEAMS_VW_UPD).'/'.md5($this->data['team_id']));
      }else{
        self::saveRecord();
