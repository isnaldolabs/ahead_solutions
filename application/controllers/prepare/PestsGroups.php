...
class PestsGroups extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_PESTS_GROUPS;
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/PestsGroups_model', 'ds_model');

    $this->data['view_title'] = 'Grupos de Pragas';
    $this->data['view_title_rec'] = 'Grupo';
    $this->data['menu_active'] = MENU_PREPARE;
    $this->data['link_delete'] = base_url(RT_PESTS_GROUPS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_PESTS_GROUPS_VW_INS);
    $this->data['link_update'] = base_url(RT_PESTS_GROUPS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/pests_groups/vw_pests_groups');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/pests_groups/vw_pests_groups_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PESTS_GROUPS_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/pests_groups/vw_pests_groups_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PESTS_GROUPS_VW_UPD).'/'.md5($this->data['group_id']));
      }else{
        self::saveRecord();
