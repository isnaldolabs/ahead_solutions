...
class ProtocolsRatings extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_PROTOCOLS_RATINGS;
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/ProtocolsRatings_model', 'ds_model');

    $this->data['view_title'] = 'Classificação dos Experimentos';
    $this->data['view_title_rec'] = 'Classificação';
    $this->data['menu_active'] = MENU_PREPARE;

    $this->data['link_delete'] = '';
    $this->data['link_insert'] = '';
    $this->data['link_update'] = base_url(RT_PROTOCOLS_RATINGS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/protocols_ratings/vw_protocols_ratings');
  ...
  public function ct_vw_update($key){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/protocols_ratings/vw_protocols_ratings_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_PROTOCOLS_RATINGS_VW_UPD).'/'.md5($this->data['rating_id']));
      }else{
        self::saveRecord();
