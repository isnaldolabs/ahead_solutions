...
class TypesTests extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_TYPES_TESTS;
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/TypesTests_model', 'ds_model');

    $this->data['view_title'] = 'Tipos de Ensaios';
    $this->data['view_title_rec'] = 'Tipo de Ensaio';
    $this->data['menu_active'] = MENU_PREPARE;
    $this->data['link_delete'] = base_url(RT_TYPES_TESTS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_TYPES_TESTS_VW_INS);
    $this->data['link_update'] = base_url(RT_TYPES_TESTS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/types_tests/vw_types_tests');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/types_tests/vw_types_tests_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_TYPES_TESTS_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_test_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->test_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/types_tests/vw_types_tests_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_TYPES_TESTS_VW_UPD).'/'.md5($this->data['test_id']));
      }else{
        self::saveRecord();
