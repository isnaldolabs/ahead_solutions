...
class ResearchSubLines extends CI_Controller {

  protected $data = array();
  protected $link_redirect = RT_RESEARCH_SUBLINES;
  ...
  private function prepareFields(){
    ...
    $this->data['input_line_id']      = fl_line_id($this->session->flashdata('set_value_line_id'), $this->session->flashdata('set_error_line_id'));
  ...
  private function flashRecord(){
    ...
    $this->session->set_flashdata('set_value_description',  $this->data['description']);
  }

  private function flashErrors(){
    $this->session->set_flashdata('set_error_license_id',   form_error('edt_license_id'));
    $this->session->set_flashdata('set_error_line_id',      form_error('edt_line_id'));
    $this->session->set_flashdata('set_error_name',         form_error('edt_name'));
    $this->session->set_flashdata('set_error_description',  form_error('edt_description'));
  }
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/ResearchSubLine_model', 'ds_model');

    $this->data['view_title'] = 'Sublinhas de Pesquisa';
    $this->data['view_title_rec'] = 'Sublinha de Pesquisa';
    $this->data['menu_active'] = MENU_PREPARE;
    $this->data['link_delete'] = base_url(RT_RESEARCH_SUBLINES_DB_DEL);
    $this->data['link_insert'] = base_url(RT_RESEARCH_SUBLINES_VW_INS);
    $this->data['link_update'] = base_url(RT_RESEARCH_SUBLINES_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/research_sublines/vw_research_sublines');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/research_sublines/vw_research_sublines_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_RESEARCH_SUBLINES_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_subline_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->subline_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/research_sublines/vw_research_sublines_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_RESEARCH_SUBLINES_VW_UPD).'/'.md5($this->data['subline_id']));
      }else{
        self::saveRecord();
