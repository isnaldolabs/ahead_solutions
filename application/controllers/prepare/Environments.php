...
class Environments extends CI_Controller {
  ...
  protected $link_redirect = RT_ENVIRONMENTS;
  ...
  private function prepareFields(){
    ...
    $this->data['input_name']           = fl_name10($this->session->flashdata('set_value_name'), $this->session->flashdata('set_error_name'));
  }
  ...
  private function checkRecord(){
    ...
    $this->form_validation->set_rules(array(rule_name10()));
    return $this->form_validation->run();
  }

  private function saveRecord(){
    if(isset($this->data['environment_id'])){
  ...
  public function __construct(){
    ...
    fb_get_logged();
    $this->output->cache(0);
    $this->load->model('prepare/Environment_model', 'ds_model');

    $this->data['view_title'] = 'Ambientes de Produção';
    $this->data['view_title_rec'] = 'Ambiente de Produção';
    $this->data['menu_active'] = MENU_PREPARE;
    $this->data['link_delete'] = base_url(RT_ENVIRONMENTS_DB_DEL);
    $this->data['link_insert'] = base_url(RT_ENVIRONMENTS_VW_INS);
    $this->data['link_update'] = base_url(RT_ENVIRONMENTS_VW_UPD);
  }
  ...
  public function index(){
    ...
    $this->load->view('prepare/environments/vw_environments');
  ...
  public function ct_vw_insert(){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/environments/vw_environments_ins', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_insert(){
    ...
    elseif(isset($_POST[BTN_POST_SAVE])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_ENVIRONMENTS_VW_INS));
      }else{
        self::saveRecord();
  ...
  public function ct_vw_update($key){
    if(($key!=NULL) and ($this->session->flashdata('set_value_environment_id')!=NULL) ){
      self::prepareFields();
    }else{
      $loRecord = $this->ds_model->get_by_key($key);
      if ($loRecord[0]->environment_id > 0){
    ...
    $this->load->view('patterns/v_header');
    $this->load->view('patterns/v_navigation', $this->data);
    $this->load->view('prepare/environments/vw_environments_upd', $this->data);
    $this->load->view('patterns/v_footer');
  }
  ...
  public function ct_db_update(){
    if(isset($_POST[BTN_POST_RETURN])){
      ...
      if($lbCheck==FALSE){
        self::flashErrors();
        redirect(base_url(RT_ENVIRONMENTS_VW_UPD).'/'.md5($this->data['environment_id']));
      }else{
        self::saveRecord();
