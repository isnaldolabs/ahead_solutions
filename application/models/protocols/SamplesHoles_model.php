...
class SamplesHoles_model extends CI_Model {

  protected $main_table = 'samples_holes';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(hole_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_holes', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('hole_id', $poRecord['hole_id']);
    return $this->db->update('samples_holes', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(hole_id)', $piKey);
      return $this->db->delete('samples_holes');
    }
  }
