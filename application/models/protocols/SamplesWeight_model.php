...
class SamplesWeight_model extends CI_Model {

  protected $main_table = 'samples_weight';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(weight_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_weight', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('weight_id', $poRecord['weight_id']);
    return $this->db->update('samples_weight', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(weight_id)', $piKey);
      return $this->db->delete('samples_weight');
    }
  }
