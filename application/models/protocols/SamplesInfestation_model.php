...
class SamplesInfestation_model extends CI_Model {

  protected $main_table = 'samples_infestation';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(infestation_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_infestation', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('infestation_id', $poRecord['infestation_id']);
    return $this->db->update('samples_infestation', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(infestation_id)', $piKey);
      return $this->db->delete('samples_infestation');
    }
  }
