...
class SamplesDiameters_model extends CI_Model {

  protected $main_table = 'samples_diameters';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(diameter_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_diameters', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('diameter_id', $poRecord['diameter_id']);
    return $this->db->update('samples_diameters', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(diameter_id)', $piKey);
      return $this->db->delete('samples_diameters');
    }
  }
