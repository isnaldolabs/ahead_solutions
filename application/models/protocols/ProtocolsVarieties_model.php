...
class ProtocolsVarieties_model extends CI_Model {

  protected $main_table = 'protocols_varieties';
  ...
  function get_all($paParams){
    $this->db->select('a.treatment_id, a.protocol_id, a.parcel_id, a.license_id, d.plot_code,'.
                      'b.variety_id, b.name variety_name, c.dt_planting, c.area, c.parcel_code,'.
    ...
    $this->db->join('varieties b', '(b.license_id=c.license_id and b.variety_id=c.variety_id)', 'left');
  ...
  function get_list_parcels_varieties($paParams){
    ...
    $this->db->where('a.license_id', $paParams['license_id']);
    $this->db->where('a.protocol_id', $paParams['protocol_id']);
    $this->db->where('not exists (select pv.parcel_id '.
                                  ' from protocols_varieties pv '. 
                                 ' where pv.license_id=a.license_id and pv.protocol_id=a.protocol_id and pv.parcel_id=b.parcel_id)');
