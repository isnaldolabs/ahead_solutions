...
class SamplesPests_model extends CI_Model {

  protected $main_table = 'samples_pests';
  ...
  function get_list_pests($paParams){
    $this->db->select('a.pest_id column_key, a.name column_name');
  ...
  function add($poRecord){
    return $this->db->insert('samples_pests', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('sample_id', $poRecord['sample_id']);
    return $this->db->update('samples_pests', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(sample_id)', $piKey);
      return $this->db->delete('samples_pests');
    }
  }
