...
class SamplesAnalyze_model extends CI_Model {

  protected $main_table = 'samples_analyze';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(analyze_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_analyze', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('analyze_id', $poRecord['analyze_id']);
    return $this->db->update('samples_analyze', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(analyze_id)', $piKey);
      return $this->db->delete('samples_analyze');
    }
  }
