...
class SamplesStems_model extends CI_Model {

  protected $main_table = 'samples_stems';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(stem_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_stems', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('stem_id', $poRecord['stem_id']);
    return $this->db->update('samples_stems', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(stem_id)', $piKey);
      return $this->db->delete('samples_stems');
    }
  }
