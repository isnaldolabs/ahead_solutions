...
class SamplesPith_model extends CI_Model {

  protected $main_table = 'samples_pith';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(pith_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_pith', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('pith_id', $poRecord['pith_id']);
    return $this->db->update('samples_pith', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(pith_id)', $piKey);
      return $this->db->delete('samples_pith');
    }
  }
