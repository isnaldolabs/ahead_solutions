...
class SamplesTillers_model extends CI_Model {

  protected $main_table = 'samples_tillers';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(tiller_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_tillers', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('tiller_id', $poRecord['tiller_id']);
    return $this->db->update('samples_tillers', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(tiller_id)', $piKey);
      return $this->db->delete('samples_tillers');
    }
  }
