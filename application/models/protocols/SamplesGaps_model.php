...
class SamplesGaps_model extends CI_Model {

  protected $main_table = 'samples_gaps';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(gap_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_gaps', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('gap_id', $poRecord['gap_id']);
    return $this->db->update('samples_gaps', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(gap_id)', $piKey);
      return $this->db->delete('samples_gaps');
    }
  }
