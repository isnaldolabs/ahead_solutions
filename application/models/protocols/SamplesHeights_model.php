...
class SamplesHeights_model extends CI_Model {

  protected $main_table = 'samples_heights';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(height_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_heights', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('height_id', $poRecord['height_id']);
    return $this->db->update('samples_heights', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(height_id)', $piKey);
      return $this->db->delete('samples_heights');
    }
  }
