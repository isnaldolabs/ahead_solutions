...
class ProtocolsProductsSettings_model extends CI_Model {

  protected $main_table = 'protocols_products_settings';
  ...
  function get_list_doses($paParams){
    $this->db->select('a.dose_id column_key, concat(a.dose, " - ", mu.name) column_name');
    $this->db->from('doses a');
    $this->db->join('measure_units mu', '(mu.license_id=a.license_id and mu.measure_id=a.measure_id)');
    $this->db->where('a.license_id', $paParams['license_id']);
    $this->db->order_by('a.dose', '');
    return $this->db->get()->result();
  }
