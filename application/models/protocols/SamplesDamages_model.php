...
class SamplesDamages_model extends CI_Model {

  protected $main_table = 'samples_damages';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(damage_id)', $psKey);
  ...
  function add($poRecord){
    return $this->db->insert('samples_damages', $poRecord);
  }

  function upd($poRecord){
    $this->db->where('damage_id', $poRecord['damage_id']);
    return $this->db->update('samples_damages', $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(damage_id)', $piKey);
      return $this->db->delete('samples_damages');
    }
  }
