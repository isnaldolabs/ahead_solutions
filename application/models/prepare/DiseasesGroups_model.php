...
class DiseasesGroups_model extends CI_Model {

  protected $main_table = 'diseases_groups';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(group_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('group_id', $poRecord['group_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(group_id)', $piKey);
