...
class Compounds_model extends CI_Model {

  protected $main_table = 'compounds';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(compound_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('compound_id', $poRecord['compound_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(compound_id)', $piKey);
