...
class Designing_model extends CI_Model {

  protected $main_table = 'designing';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(designing_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('designing_id', $poRecord['designing_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(designing_id)', $piKey);
