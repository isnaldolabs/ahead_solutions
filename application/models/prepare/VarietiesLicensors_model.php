...
class VarietiesLicensors_model extends CI_Model {

  protected $main_table = 'varieties_licensors';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(licensor_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('licensor_id', $poRecord['licensor_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(licensor_id)', $piKey);
