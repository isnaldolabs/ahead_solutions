...
class MeasureUnit_model extends CI_Model {

  protected $main_table = 'measure_units';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(measure_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('measure_id', $poRecord['measure_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(measure_id)', $piKey);
