...
class Environment_model extends CI_Model {
  ...
  protected $main_table = 'environments';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(environment_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('environment_id', $poRecord['environment_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(environment_id)', $piKey);
