...
class PlantingSchemes_model extends CI_Model {

  protected $main_table = 'planting_schemes';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(scheme_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('scheme_id', $poRecord['scheme_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(scheme_id)', $piKey);
