...
class ChecksDescriptions_model extends CI_Model {

  protected $main_table = 'checks_descriptions';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(description_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('description_id', $poRecord['description_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(description_id)', $piKey);
