...
class ChecksMethods_model extends CI_Model {

  protected $main_table = 'checks_methods';
  ...
  function get_by_key($psKey){
    $this->db->where('md5(method_id)', $psKey);
    return $this->db->get($this->main_table)->result();
  }
  ...
  function upd($poRecord){
    $this->db->where('method_id', $poRecord['method_id']);
    return $this->db->update($this->main_table, $poRecord);
  }

  function delete($piKey=NULL){
    if($piKey){
      $this->db->where('md5(method_id)', $piKey);
